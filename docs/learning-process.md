...
Curriculum generation advances one bounded stage per request. Private checkpoints survive reloads and provider errors; a per-user lease prevents overlapping writers. Only a complete, validated graph is committed. The browser continues the selected topic across stages, and generation checks stop continuation after a reset. An expansion is bounded to 128 new concepts and each provider call permits at most three structured-output attempts. These are operational limits, not permission to drop prerequisites.

Cycle prevention uses the graph itself: generation receives the target and every concept that already depends on it, and the server checks every proposed edge. A self-dependency or indirect cycle rolls back the entire failed stage and checkpoints a repair of the affected concept. Repair rewrites its knowledge and prerequisite list consistently; it does not silently delete a necessary edge. After two unsuccessful repairs of that concept, preparation switches to a simpler, different unpublished boss in the same topic and subtopic. At most two such restarts are allowed before a recoverable preparation error. Existing saved concepts and earned progress remain unchanged. Older drafts stuck at a cyclic match automatically enter this recovery flow when retried; no data reset is needed.
