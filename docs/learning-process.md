...
Generation advances one bounded stage per request. Every generated boss, knowledge set, dependency list, and matched edge is validated and written to the private learning graph immediately. Saving is independent of question selection: the next request reloads the graph and can select any unfinished node to continue from its durable preparation state. Unfinished nodes remain hidden and there is no separate curriculum draft. A per-user lease prevents overlapping writers, and generation checks stop continuation after a reset. Each patch is bounded to 129 nodes and each generation uses one provider call, with no audit, repair, revision or automatic retry. These operational limits remain separate from cycle handling.
