...
Edit [`game-balance.json`](../supabase/functions/_shared/game-balance.json) for gameplay prices, rewards, unit and equipment stats, building effects, production, trading, towers, doctrines, and campaign tuning. The browser and Edge function import this same file. Building and unit names match the UI, and every purchase cost names its resource explicitly.
