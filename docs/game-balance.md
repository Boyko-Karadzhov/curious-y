...
The `learningValue` section also feeds PostgreSQL. After editing the JSON, run `node scripts/sync-game-balance.mjs` to refresh the generated migration, and `node scripts/sync-game-balance.mjs --check` to verify it. Once a migration is deployed, create a new forward migration for later tuning changes rather than rewriting deployed history. Deploy the Edge function and database migration together.
