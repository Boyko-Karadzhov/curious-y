...
Swordsman is the first integrated generated character. The other 19 units retain their existing artwork during a staged migration. Tiny Swords buildings and terrain are also still in use. This is a working first character and pipeline, not a finished roster-wide art replacement.
...
Suggested rollout: Archer and Medic next (contrast against Swordsman), then one complete mounted unit and Catapult. These test humanoids, supports, mounts and machines before the remaining roster. Review those five together on the battlefield before scaling production. Subsequently replace buildings/environment with the same art direction to eliminate the remaining pixel/painted style mix.
...
Local review: run the Vite dev server, then open `/docs/art/review.html`. This developer review page is not part of the production game bundle.
