...
The existing `learning/index.ts` route already parses intent, obtains database time, checks generation and request identity, retries optimistic revisions, and commits through the service role. The new command follows that route. It accepts no client stats, balances, victory, modifiers or clock values. Server catch-up derives its step bound from the battle snapshot. A long absence resolves at most one explicitly started fight and pays its victory once; it never auto-starts another stage. Client snapshot conversion also covers GET, command, reward and reset responses without importing browser ownership into signed-in accounts.
