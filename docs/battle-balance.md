...
Battles retain fivefold playback: one 0.25-second deterministic simulation step takes 50 milliseconds. The maximum is 450 simulation seconds / 90 real seconds. Damage, healing, movement and recruitment scale together; UI times are real seconds. Older frozen battles keep their original rules and clocks.
...
| 2 | militia; Keep/buildings 1, training 1 | defeat | 21.95 |
| 2 | militia, slinger; Keep/buildings 1, training 1 | victory | 16.2 |
| 5 | militia, slinger; Keep/buildings 1, training 1 | victory | 19.55 |
...
At chapter transitions (stages 11/21/31/41), the previous roster tier with building levels 2/3/4/5 and training 1 wins in 19.5–20.8 seconds. The matching new tier wins faster. The unit-level combat suite additionally checks that each fresh successor beats a predecessor with maximum training/stars at equal building investment.
...
Run npm test, npm run test:db, npm run build, and npm run lint. No new migration is needed for rules 8. Deploy the learning Edge Function and push main for GitHub Pages. Historical fixtures continue to verify older battle outcomes and reward collection.
