...
- The answer transaction locks the account before the question, legacy stats, and concepts. It records versioned learning-value rewards and a unique reward event atomically; explicit collection credits the saved weighted Resource lines. Repeating the same answer returns its result; a different choice cannot replace it.
...
- Question generation reserves one account slot for up to two minutes; finalization checks the reservation and generation. An eligible active question is reused within its topic; topic changes follow the server topic-selection contract. Old questions are quarantined from new scoring. Registry reads fail closed and precede model-output eligibility validation.
...
Schema 3 and battle rules 3 add military specialties, bounded Medic healing and frozen Treasury payouts. Library reconciliation reads protected mastery/alias records and updates only derived game state under the account lock; no currency or client command can grant Library levels. The SQL command/goal allowlists permit Academy and Treasury purchases and reject Library/Forge purchases. See [the eligibility, migration, combat and release contracts](../castle-progression.md).
