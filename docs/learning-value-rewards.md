...
2. Review `npx supabase db push --dry-run`; apply only the forward `20260906050000_learning_value.sql` migration. Step 3 must already be installed. Do not replay scoring or backfill credits.
...
Release verification on 2026-09-06: 352 tests passed across 39 suites; 1,456 database checks passed in PGlite and 1,499 passed against disposable PostgreSQL 18 with real concurrent sessions. Production build and lint passed (the existing warnings above remain).
