...
export function Review(){
 ...
 useEffect(()=>{if(!playing)return;const t=setInterval(()=>setPose(p=>(p+1)%12),250);return()=>clearInterval(t);},[playing]);
 useEffect(()=>{
  let disposed=false;
  ...
  const originals=SWARM_IDS.map(id=>new Promise<HTMLImageElement>(resolve=>{const image=new Image();image.onload=()=>resolve(image);image.src=unitArt(id).atlas.src;}));
  void Promise.all([Promise.all(originals),loadEquipmentArtwork(SWARM_IDS.flatMap(id=>loadouts.map(equipment=>({id,equipment}))))]).then(([images])=>{
   if(disposed)return;const c=ref.current!,g=c.getContext('2d')!;g.fillStyle='#101b2c';g.fillRect(0,0,c.width,c.height);
   const labels=['Natural','Mandibles only','Carapace only',`Matched tier ${tier}`,'Weapon 5 / Armor 1','Weapon 1 / Armor 5'];
   labels.forEach((label,i)=>{g.fillStyle='#efc889';g.font='bold 15px sans-serif';g.textAlign='center';g.fillText(label,120+i*220,30);});
   SWARM_IDS.forEach((id,row)=>{
    loadouts.forEach((equipment,col)=>{
     ...
     if(!drawEquippedUnit(g,id,equipment,pose,90))g.drawImage(images[row],pose%4*256,Math.floor(pose/4)*256,256,256,-128*s,-232*s,256*s,256*s);
     if(guides){const f=swarmFrameRig(id,pose);g.strokeStyle='#ff7979';g.lineWidth=1;g.strokeRect((f.shell[0]-128)*s,(f.shell[1]-232)*s,f.shell[2]*s,f.shell[3]*s);for(const p of [f.upper,f.lower]){g.fillStyle='#fff';g.beginPath();g.arc((p[0]-128)*s,(p[1]-232)*s,2,0,Math.PI*2);g.fill();}}
   ...
   for(let i=0;i<3;i++) {
    ...
    if(i===0)zg.drawImage(images[0],pose%4*256,Math.floor(pose/4)*256,256,256,-128*s,-232*s,256*s,256*s);
    else drawEquippedUnit(zg,'hatchling',{weapon:tier,armor:tier},pose,height);
 ...
 const audit=async()=>{
  ...
  for(const id of SWARM_IDS)for(let weapon=0;weapon<=5;weapon++)for(let armor=0;armor<=5;armor++) {
   ...
   for(let frame=0;frame<12;frame++) {
    ...
    if(!drawEquippedUnit(g,id,{weapon,armor},frame,unitArt(id).idleHeight))missing++;
    g.restore();const pixels=g.getImageData(0,0,320,320).data;
    for(let i=0;i<256;i++)if(pixels[((32*320+32+i)*4)+3]>8||pixels[((287*320+32+i)*4)+3]>8||pixels[(((32+i)*320+32)*4)+3]>8||pixels[(((32+i)*320+287)*4)+3]>8){clipped++;break;}
    count++;
   }
  }
 ...
 return <main style={{maxWidth:1400,margin:'auto',padding:24,color:'#dbe7f7'}}>
  ...
  <div style={{display:'flex',gap:20,alignItems:'center',margin:'24px 0',flexWrap:'wrap'}}>
   ...
   <label>Frame <input aria-label="Frame" type="range" min={0} max={11} value={pose} onChange={e=>{setPlaying(false);setPose(+e.target.value);}}/>{pose} · {pose<4?'Idle':pose<8?'Walk':'Bite'}</label>
   ...
   {[0,4,8,9,10,11].map(frame=><button key={frame} onClick={()=>{setPlaying(false);setPose(frame);}}>Pose {frame}</button>)}
