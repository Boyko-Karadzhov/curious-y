...
const initial=()=>{const s=newKingdom();s.castle=4;s.buildings.forge=40;s.forge.count=390;for(const topic of TOPICS)s.tokens[topic]=100;for(const [i,c]of UNIT_CLASSES.entries()){s.buildings[c.building]=1;const unit=UNITS.find(u=>u.unitClass===c.id&&u.tier===3)!;s.units[c.id]={unitId:unit.id,investedXP:0,locked:false};s.armySlots[i]=c.id;s.forge.equipped[`${c.id}:weapon`]={id:`initial-${c.id}`,unitClass:c.id,slot:'weapon',tier:1,bonus:{stat:'hp',target:c.id,value:5}};}return s;};
function Review(){
    ...
    useEffect(()=>{const c=canvas.current!,renderer=new BattleRenderer(c,c.getContext('2d')!);let s={...state,battle:createBattle(state,30)};renderer.update(s.battle!,true);const timer=setInterval(()=>{for(let i=0;i<2;i++)s=applyAction(s,{type:'tick'});renderer.update(s.battle!,!s.battle!.result);},100);return()=>{clearInterval(timer);renderer.dispose();};},[battleKey]);
    useEffect(()=>{
        ...
        const originals=roster.map(unit=>{const img=new Image();img.src=unitArt(unit.id).atlas.src;return img;});
        const begin=performance.now();
        const draw=(now:number)=>{
            if(disposed)return;ctx.clearRect(0,0,c.width,c.height);
            ...
            roster.forEach((unit,i)=>{
                ...
                if(originals[i].complete&&originals[i].naturalWidth)ctx.drawImage(originals[i],index%4*256,Math.floor(index/4)*256,256,256,-art.atlas.anchorX*size,-art.atlas.anchorY*size,size,size);
                ctx.restore();ctx.save();ctx.translate(x+42,160);
                if(unit.unitClass==='siege')drawSiegeAmmunition(ctx,tier,40,running?seconds*3:0);
                else if(!drawEquippedUnit(ctx,unit.id,{weapon:tier,armor},index,65)&&originals[i].complete&&originals[i].naturalWidth)ctx.drawImage(originals[i],index%4*256,Math.floor(index/4)*256,256,256,-art.atlas.anchorX*size,-art.atlas.anchorY*size,size,size);
    ...
    return <main style={{maxWidth:1100,margin:'0 auto',padding:'30px 16px',color:'#dce5f5'}}><header style={{marginBottom:24}}><p style={{color:'#eabb72',fontSize:11,letterSpacing:3}}>FORGE · PLAYABLE REVIEW</p><h1 style={{fontSize:32,fontWeight:800,margin:'10px 0'}}>A new purpose for every resource.</h1><p style={{color:'#97a8c0'}}>Isolated review state · This page does not spend your account’s resources.</p></header><ForgePanel state={state} blocked={false} perform={async(action:Action)=>{setState(s=>applyAction(s,action,{requestId:crypto.randomUUID(),draws:Array.from(crypto.getRandomValues(new Uint32Array(6)),n=>n/4294967296)}));return true;}}/>
        <details style={{margin:'24px 0'}}><summary style={{cursor:'pointer',padding:12}}>Equipment in army preparation and unit collection</summary><h2 style={{fontSize:20,fontWeight:800}}>Prepare your army</h2><ArmyPreparation state={state} preparation={createBattle(state,30)} active={false} blocked={false} perform={async action=>{setState(s=>applyAction(s,action));return true;}}/><UnitRoster state={state}/></details>
...
if(import.meta.env.DEV){const root=createRoot(document.getElementById('root')!);root.render(<Review/>);import.meta.hot?.dispose(()=>root.unmount());}
