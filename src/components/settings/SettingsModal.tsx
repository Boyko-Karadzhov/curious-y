...
export const SettingsModal: React.FC<SettingsModalProps> = ({ isOpen, onClose, onResetProgress }) => {
  const { user } = useAuth();
  const { settings, saving, updateSettings, clearApiKey, testConnection } = useSettings();
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-xs animate-fade-in">
      <div className="w-full max-w-lg overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-2xl">
        ...
        <div className="space-y-4 p-6">
          <div className="flex gap-3 rounded-2xl border border-emerald-200 bg-emerald-50 p-4">
            ...
            <div>
              ...
              <p className="mt-1 text-xs leading-relaxed text-emerald-800">
                Questions, answer checks, rewards, and mastery updates run in the protected backend. Your browser can read progress but cannot write it.
              </p>
            </div>
          </div>
                ...
                <div>
                  <div className="flex items-center gap-2">
                    ...
                    <span className={`rounded-full px-2 py-0.5 text-[10px] font-bold ${settings.hasApiKey ? 'bg-emerald-100 text-emerald-700' : 'bg-amber-100 text-amber-700'}`}>
                      {settings.hasApiKey ? 'Key saved' : 'Key required'}
                    </span>
                  </div>
          ...
          <div className="flex items-center justify-between gap-4 border-t border-slate-100 pt-4">
            <div>
              ...
              <p className="mt-0.5 text-xs text-slate-500">Permanently reset your history, mastery, and kingdom stats.</p>
            </div>
        ...
        <div className="flex justify-end gap-2 border-t border-slate-100 bg-slate-50 px-6 py-4">
          ...
          <button type="button" onClick={handleSave} disabled={saving || !apiKey.trim()} className="inline-flex items-center gap-1.5 rounded-xl bg-slate-800 px-5 py-2 text-sm font-semibold text-white hover:bg-slate-900 disabled:opacity-50">
