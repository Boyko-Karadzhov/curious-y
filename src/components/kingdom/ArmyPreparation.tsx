...
import { Action, battleSpeed, battleSeconds, ArmySlots, Battle, Kingdom, UnitId, UNITS, eligibleUnit, effectiveOwnedUnit, effectDescription, unitAbilityDescription, unitDamagePerSecond } from '../../lib/kingdom/game';
...
export function ArmyPreparation({ state, preparation, active, blocked, perform }: {
  ...
  const stats = unit ? (active ? preparation.config.slots.find(u => u?.id === candidate) : effectiveOwnedUnit(state, unit.id)) : null;
  useEffect(() => {
    if (slot === null) return;
  ...
  return <>
    ...
    {slot !== null && <div id="army-slot-details" ref={details} tabIndex={-1} role="region" aria-label={`Army slot ${slot + 1} details`} onKeyDown={event => { if (event.key === 'Escape') close(); }} className="mt-4 rounded-2xl border border-slate-600 bg-slate-800 p-4">
      ...
      {unit && stats ? <div className="mb-4">
        <div className="flex items-center gap-2"><UnitPortrait id={unit.id} /><div><h4 className="font-bold">{unit.name} · Level {state.units[unit.id]?.level ?? 1} · {state.units[unit.id]?.stars ?? 1}★</h4><p className="text-sm text-slate-300">{unit.role}</p><p className="text-xs text-sky-200">{unitAbilityDescription(unit.id, preparation.config.rulesVersion)}</p></div></div>
        ...
        {stats.healBudget ? <p className="mb-2 text-sm">Healing: {Number(((stats.healPerSecond ?? 0) * battleSpeed(preparation.config.rulesVersion)).toFixed(2))} HP/sec · {stats.healBudget} HP per Medic maximum</p> : null}
