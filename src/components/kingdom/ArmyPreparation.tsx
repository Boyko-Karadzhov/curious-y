...
export function ArmyPreparation({ state, preparation, active, blocked, perform }: {
  ...
  const available = owned.filter(u => eligibleUnit(state, u.id) && !state.armySlots.some(id => id && state.units[id].unitId === u.unitId));
  ...
  return <>
    ...
    <div className="mt-4 grid max-w-xl grid-cols-2 gap-3 sm:grid-cols-5">
      {state.armySlots.map((id, index) => <button key={index} id={`army-square-${index}`} ref={node => { squares.current[index] = node; }} type="button"
        ...
        {id ? <UnitPortrait id={state.units[id].unitId} /> : <Plus aria-hidden="true" className="my-5 h-10 w-10 text-slate-400" />}
    ...
    {slot !== null && <div id="army-slot-details" ref={details} tabIndex={-1} role="region" aria-label={`Army slot ${slot + 1} details`} onKeyDown={event => { if (event.key === 'Escape') close(); }} className="mt-4 rounded-2xl border border-slate-600 bg-slate-800 p-4">
      ...
      {unit && stats ? <div className="mb-4">
        <div className="flex items-center gap-2"><UnitPortrait id={unit.unitId} /><div><h4 className="font-bold">{unit.name} · Level {recruitLevel(state.units[unit.id])}</h4><p className="text-sm text-slate-300">{unit.role}</p><p className="text-xs text-sky-200">{unitAbilityDescription(unit.unitId, preparation.config.rulesVersion)}</p></div></div>
        <p className="mb-3 text-sm text-emerald-200">{active && preparation.config.rulesVersion < 3 ? 'Legacy battle: original stats; specialties begin next battle.' : active ? `Battle snapshot: ${Number(((stats.armor ?? 0) * 100).toFixed(2))}% armor · ${stats.range} reach · ${battleSeconds(preparation, stats.attackInterval ?? 0)}s reload` : effectDescription(unit.building, state.buildings[unit.building])}</p>
      ...
      {active ? <p className="text-sm text-amber-200">Finish or retreat from the battle to change units.</p> : <>
        <div className="grid grid-cols-2 gap-2 sm:grid-cols-4 lg:grid-cols-5" role="group" aria-label="Available units">{owned.filter(u => !owned.some(other => other.unitId === u.unitId && (state.units[other.id].investedXP > state.units[u.id].investedXP || state.units[other.id].investedXP === state.units[u.id].investedXP && other.id < u.id))).map(u => {
          const assignedElsewhere = state.armySlots.some((id,i) => i !== slot && id && state.units[id].unitId === u.unitId);
          return <button type="button" key={u.id} disabled={blocked || !eligibleUnit(state, u.id) || assignedElsewhere} aria-pressed={candidate === u.id} onClick={() => void assign(u.id)} className="flex min-h-28 min-w-0 flex-col items-center justify-center rounded-xl border border-slate-500 p-2 text-sm enabled:hover:bg-slate-700 focus-visible:outline focus-visible:outline-2 focus-visible:outline-amber-300 aria-pressed:border-sky-300 aria-pressed:bg-sky-900 disabled:grayscale disabled:opacity-40"><UnitPortrait id={u.unitId} /><span className="font-bold">{u.name}{assignedElsewhere ? ' · assigned' : ''}</span></button>;
        })}</div>
