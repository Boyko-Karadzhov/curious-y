      ...
      <section className="grid gap-4 sm:grid-cols-2" aria-label="Knowledge and economy buildings">
        {BUILDING_DEFINITIONS.filter(b => !BUILDINGS.some(m => m.id === b.id)).map(spec => {
          ...
          return <article key={spec.id} id={`kingdom-building-${spec.id}`} tabIndex={-1} className="scroll-mt-4 rounded-2xl border border-slate-200 bg-white p-5">
            ...
            </> : spec.mode === 'future' ? <p className="mt-3 text-sm text-slate-500">Planned at Keep {spec.unlock}. Equipment and crafting depend on section 10; this building cannot be constructed yet.</p> : <>
              <p className="mt-3 text-xs text-slate-500">Victory Gold only: 2% per level, maximum 10%, rounded down. Frozen at battle start and collected once. Offline production depends on section 22.</p>
