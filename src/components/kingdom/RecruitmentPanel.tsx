...
import { Action, Kingdom, RecruitingBuilding, recruitmentCost, recruitmentOdds, formatOdds, formatCost, canAfford, missingCost, RECRUITMENT, unitDefinition } from '../../lib/kingdom/game';
...
export function RecruitmentPanel({ state, id, blocked, perform }: {
  state: Kingdom;
  ...
  perform: (action: Action) => Promise<boolean>;
    ...
    return <section aria-label="Recruitment" className="mt-4 space-y-3 border-t border-slate-300 pt-4">
        ...
        {!canAfford(state,cost) && <p className="text-sm">Need {formatCost(missingCost(state,cost))} more. Collect Food from the Farm or trade at the Market.</p>}
