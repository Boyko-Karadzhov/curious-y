...
import { BUILDINGS, CAMPAIGN, Kingdom, castleCost } from '../../lib/kingdom/game';
...
export const QuestRail: React.FC<{ state: Kingdom; onCastle: () => void }> = ({ state, onCastle }) => {
  ...
  return <aside className="space-y-4 lg:sticky lg:top-20 lg:self-start" aria-label="Castle progress">
    ...
    <section className="game-rail-card">
      <h3 className="flex items-center gap-2 font-bold text-white"><Flag className="h-4 w-4 text-amber-300" /> PvE campaign</h3>
      <p className="mt-2 text-xs text-slate-300">{state.cleared}/{CAMPAIGN.length} fronts cleared</p>
      <p className="mt-2 text-xs text-slate-400">{state.cleared < CAMPAIGN.length ? `Next: ${CAMPAIGN[state.cleared]}` : 'Campaign complete. Replay any front.'}</p>
    </section>
  </aside>;
};
