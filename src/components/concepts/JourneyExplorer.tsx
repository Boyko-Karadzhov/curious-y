            ...
            <div className={`journey-workspace ${detailsOpen && node ? 'journey-workspace-open' : ''}`}>
                ...
                {detailsOpen && node && <aside key={node.id} id={detailId} className="journey-detail" aria-label="Concept details">
                    {node ? <>
                        ...
                        {node.kind === 'concept' && <div className="journey-mastery"><strong>{conceptMastery(node)}% toward mastery</strong><progress aria-label={`${node.title} mastery`} max={100} value={conceptMastery(node)} /><p>Complete all seven dimensions, then one challenge at each reasoning complexity.</p></div>}
                        <p className="journey-evidence-note">{node.kind === 'boss' ? node.status === 'completed' ? 'Challenge conquered.' : 'Your prerequisites are proficient. Put them together to answer this question.' : proficient(node, node.progress) ? node.status === 'mastered' ? 'Mastered. Keep your understanding fresh with practice.' : 'Proficient. Reasoning questions deepen your mastery.' : 'Each question builds on the last. Your next step is chosen automatically.'}</p>
