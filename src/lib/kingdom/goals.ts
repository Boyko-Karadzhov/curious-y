...
export function parseGoal(value: unknown): ProgressionGoal | null {
    ...
    if (goal.type === 'recruit') {
        return isRecruitingBuilding(goal.id) && Number.isSafeInteger(goal.count) && goal.count > 0 ? {
            type:'recruit',
            id:goal.id,
            count:goal.count
        } : null;
...
export function goalOptions(state: Kingdom): ProgressionGoal[] {
    return [
        ...BUILDING_DEFINITIONS.filter(b=>isRecruitingBuilding(b.id) && state.buildings[b.id]>0).map(b=>({
            type:'recruit' as const,
            id:b.id as RecruitingBuilding,
            count:state.recruitCount[b.id as RecruitingBuilding]+1
        })),
        ...BUILDING_DEFINITIONS.filter(b => b.mode === 'purchase' && state.buildings[b.id] < (isRecruitingBuilding(b.id) || b.id === 'forge' ? 1 : b.cap)).map(b => ({
            type: 'building' as const,
            id: b.id,
            level: state.buildings[b.id] + 1
        })),
        ...(state.castle < MAX_LEVEL ? [{
...
export function goalProgress(state: Kingdom, goal: ProgressionGoal) {
    if (goal.type === 'recruit') {
        ...
        return {
            ...
            requiredCastle:BUILDING_DEFINITIONS.find(b=>b.id===goal.id)!.unlock,
            blocker,
            affordable,
            ...
            complete:state.recruitCount[goal.id]>=goal.count,
            invalid:state.recruitCount[goal.id]+1<goal.count
        };
    }

    const current = goal.type === 'castle' ? state.castle : state.buildings[goal.id];
    const action: UpgradeAction = goal.type === 'castle' ? { type: 'castle' } : {
        type: 'building',
        id: goal.id
    };
    return {
        ...upgradeStatus(state, action),
        action,
        complete: current >= goal.level,
        invalid: current + 1 < goal.level
    };
}
