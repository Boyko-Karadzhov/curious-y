...
export function saveLearningPath(userId: string, questionId: string, path: LearningPath) {
  try { localStorage.setItem(`curious_y_learning_path_${userId}`, JSON.stringify({ questionId, path })); } catch { /* In-memory navigation still works. */ }
}
export function restoreLearningPath(userId: string, questionId: string, topic: string): LearningPath {
  ...
  try {
    ...
    if (saved?.questionId !== questionId || !saved.path) return fallback;
    const path = saved.path;
    if (path.kind === 'random') return { kind: 'random' };
    if (path.kind === 'topic' && typeof path.topic === 'string') return path;
    if (path.kind === 'concept' && typeof path.topic === 'string' && typeof path.nodeId === 'string') return path;
    if (path.kind === 'goal') { const goal = parseGoal(path.goal); if (goal) return { kind: 'goal', goal }; }
    if (path.kind === 'tower' && TOWERS.some(t => t.topic === path.topic) && Number.isSafeInteger(path.points) && path.points > 0) return path;
    if (path.kind === 'library' && LIBRARY_MILESTONES.includes(path.concepts)) return path;
    if (path.kind === 'forge' && Number.isSafeInteger(path.count) && path.count > 0) return path;
...
export function nextLearningStep(path: LearningPath, state: Kingdom, graph?: JourneyView): LearningStep {
  ...
  if (path.kind === 'random') return { path };
  if (path.kind === 'topic') return { path, topic: path.topic };
  if (path.kind === 'goal') {
    const progress = goalProgress(state, path.goal);
    const title = goalTitle(path.goal);
    if (progress.complete) return done(`${title}: goal complete!`);
    if (progress.invalid) return done('This goal is no longer available. Choose a new goal in Castle.');
  ...
  if (path.kind === 'library') return state.libraryConcepts >= path.concepts
    ? done('Library learning milestone reached! Continue in Castle.') : { path };
  if (path.kind === 'forge') {
    if (state.forge.count >= path.count) return done('Forge goal complete!');
  ...
  if (!graph) throw new Error('Could not check concept mastery. Please retry.');
  ...
  for (const node of graph.nodes) for (const requirement of node.requires) {
