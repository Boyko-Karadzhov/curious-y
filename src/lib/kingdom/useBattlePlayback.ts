...
export function useBattlePlayback(state: Kingdom, userId?: string) {
  ...
  if (playback.key !== key) {
    ...
    try {
      ...
      if (saved?.id === battle?.id) {
        if (Number.isFinite(saved.elapsed)) elapsed = saved.elapsed;
  ...
  useEffect(() => {
    ...
    let savedAt = 0;
    const save = () => {
      try { sessionStorage.setItem(storageKey, JSON.stringify({ id: controller.outcome.id, elapsed: controller.battle.elapsed, done: !!controller.battle.result })); }
      catch { /* Losing a viewing position cannot lose the settled battle. */ }
    };
    const render = (now: number) => {
      if (document.hidden) { last = undefined; return; }
      if (last !== undefined && controller.advance(now - last)) redraw(value => value + 1);
      last = now;
      if (controller.battle.result || now - savedAt >= 1000) { save(); savedAt = now; }
      if (!controller.battle.result) frame = requestAnimationFrame(render);
    };
    const visibility = () => {
      cancelAnimationFrame(frame);
      last = undefined;
      save();
      if (!document.hidden && !controller.battle.result) frame = requestAnimationFrame(render);
    };
    document.addEventListener('visibilitychange', visibility);
    if (!controller.battle.result) frame = requestAnimationFrame(render);
    return () => { cancelAnimationFrame(frame); document.removeEventListener('visibilitychange', visibility); save(); };
  }, [controller, storageKey]);
  const skip = useCallback(() => {
    if (!controller || controller.battle.result) return false;
    controller.finish();
