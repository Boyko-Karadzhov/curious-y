...
const score = (input: Partial<LearningValueInput> = {}, correct = true) => createLearningValueReward('q', correct, {
    Physics: .7,
    'Mathematics & Logic': .2,
    'Earth & Space': .1
}, 'Physics', {
...
describe('learning value', () => {
    ...
    it('stacks correctness, first success, reasoning and successful boss; bounds the maximum', () => {
        ...
        expect(score({ successes: 0 }).lines).toEqual([{
            key:'force',
            amount:18
        },{
            key:'runes',
    ...
    it('evaluates exact due boundaries, excludes novelty and ignores reinforcement flags', () => {
        expect(score({
            preMastery: 'mastered',
            nextDueAt: now
        }).totalKnowledge).toBe(24);
        expect(score({
            preMastery: 'mastered',
            nextDueAt: '2026-09-06T12:00:00.001Z'
        }).totalKnowledge).toBe(6);
    ...
    it('caps low-value farming across question identities and suppresses atomic and missing metadata bonuses', () => {
        ...
        expect([0,1,2,3].map(lowValueAttempts => score({
            preMastery:'mastered',
            lowValueAttempts
        }, true).totalKnowledge)).toEqual([6,3,2,1]);
        expect([0,1,2,3].map(lowValueAttempts => score({
            preMastery:'mastered',
            boss:true,
            reasoning:'derivation',
            lowValueAttempts
        }).totalKnowledge)).toEqual([10,5,3,1]);
    ...
    it('persists the 1/3/7/14/30-day review ladder; early successes preserve due dates and failure restarts at one day', () => {
        ...
        expect(advanceReview(1, 4, now, false, now)).toEqual({
            reviewStep:0,
            nextDueAt:'2026-09-07T12:00:00.000Z'
        });
    ...
    it('keeps Demo alias receipts immutable and commits attempts, successes, mastery and review together', async () => {
        ...
        expect(await answerDemoQuestion('demo', question, 0, [{
            ...concept,
            mastery:'mastered'
        }], '2030-01-01T00:00:00Z')).toEqual(a);
        ...
        const b = await answerDemoQuestion('demo', {
            ...question,
            id:'q2',
            concept:' FORCE '
        }, 0, [concept], now);
        ...
        expect((await answerDemoQuestion('demo', {
            ...question,
            demoGeneration:1
        }, 0, [concept], now)).reward?.totalKnowledge).toBe(25);
    });
    it('retries a failed Demo ledger commit without novelty loss, and a late collection cannot clear a newer receipt', async () => {
        ...
        const next = await answerDemoQuestion('demo',{
            ...question,
            id:'next'
        },0,[concept],now);
        await changeKingdom('demo',collect);
