...
describe('authoritative resource distribution', () => {
    it('allocates the source example and conserves every tiny total in canonical order', () => {
        ...
        for (let total = 0; total <= 100; total++) {
            for (const distribution of [weights, all, {
                Physics: Number.MAX_VALUE,
                Life: Number.MAX_VALUE
            }]) {
    ...
    it('filters malformed weights and uses only the verified fallback topic', () => {
        ...
        for (const malformed of [null, [], 'Physics', {
            Physics: NaN,
            Life: Infinity,
            Chemistry: 0,
            Unknown: 1
        }]) {
            expect(allocateResources(3, malformed, 'Mind & Behavior')).toEqual([{
                key: 'insight',
                amount: 3
            }]);
        }
    ...
    it('freezes Demo pending amounts, preserves old obligations, and rejects collection after reset', async () => {
        ...
        await Promise.all([1,2,3].map(() => changeKingdom('demo', {
            type: 'answer',
            id: 'weighted',
            topic: 'Physics',
            correct: true,
        ...
        expect(loadKingdom('demo').tokens).toMatchObject({
            Physics: 7,
            'Mathematics & Logic': 2,
            'Earth & Space': 1
        });
        expect(applyAction(newKingdom(), {
            type: 'answer',
            id: 'weighted',
            topic: 'Physics',
            correct: true,
        ...
        await expect(changeKingdom('demo', {
            type: 'answer',
            id: 'weighted',
            topic: 'Physics',
            correct: true,
        ...
        localStorage.setItem('curious_y_pending_reward_old-demo', JSON.stringify({
            ...question,
            reward: undefined,
            isCorrect: false
        }));
        const old = loadPendingReward('old-demo')!;
        expect(old.reward!.lines).toEqual([{
            key: 'force',
            amount: 3
        }]);
