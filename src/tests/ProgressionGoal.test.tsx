...
function props(): ProgressionGoalProps {
    return {
        ...
        onLearnTopic: vi.fn(),
        onBattle: vi.fn(),
        onNavigateUpgrade: vi.fn(async () => true)
    };
}
...
describe('Progression goals use committed Kingdom rules', () => {
    it('distinguishes affordable construction from a Castle gate and offers the prerequisite goal', () => {
        ...
        expect(() => applyAction(p.state, {
            type: 'building',
            id: 'forge'
        })).toThrow('Requires Keep (Castle) level 2.');
    ...
    it('blocks an affordable upgrade during battle, then navigates to the normal upgrade control after retreat', async () => {
        const p = props(); p.goal = {
            type: 'building',
            id: 'treasury',
            level: 2
        };
        p.state.castle = 2; p.state.buildings.barracks = 1; p.state.units.militia={
            unitId:'militia',
            investedXP:0,
            locked:false
        }; p.state.buildings.treasury=1;p.state.tokens['Society & History']=40;p.state.tokens['Mathematics & Logic']=40;p.state.gold = p.state.lifetimeGold = 80; p.state.tokens.Physics = 20;
        p.state.armySlots = ['militia', null, null, null, null];
        p.state = applyAction(p.state, {
            type: 'start',
            stage: 1
        });
        ...
        expect(p.onNavigateUpgrade).toHaveBeenCalledWith({
            type: 'building',
            id: 'treasury'
        });
    ...
    it('does not expose stale affordability while unavailable and recovers from invalid or deleted targets', () => {
        ...
        app.rerender(<ProgressionGoalCard {...p} goal={{
            type: 'building',
            id: 'barracks',
            level: 3
        }} />);
    ...
    it('links Gold deficits to battle requirements, the next stage, or the active battle', () => {
        ...
        p.state.buildings.barracks = 1; p.state.units.militia={
            unitId:'militia',
            investedXP:0,
            locked:false
        }; p.state.cleared = 10;
