...
it('continues a randomly chosen topic and its reset generation until a question is ready', async () => {
    invoke.mockResolvedValueOnce({ data: { preparing: true, topic: 'Life', generation: 7 } })
        .mockResolvedValueOnce({ data: { preparing: true, topic: 'Life', generation: 7 } })
        .mockResolvedValueOnce({ data: { question: { id: 'ready' } } });
    await expect(practiceJourney()).resolves.toEqual({ id: 'ready' });
    expect(invoke).toHaveBeenNthCalledWith(2, 'learning', { body: { action: 'journey_practice', topic: 'Life', generation: 7 } });
    expect(invoke).toHaveBeenCalledTimes(3);
});

it('stops continuation on a failed stage so the saved checkpoint can be retried', async () => {
    invoke.mockResolvedValueOnce({ data: { preparing: true, topic: 'Physics', generation: 3 } })
