...
describe('Trusted Castle command boundary', () => {
    it.each([7, 8, 9, 10, 11, 12] as const)('enforces the wall timeout for rules %s across polling and reload', version => {
        const c = context(); c.state.buildings.barracks = 1; c.state.units.militia={
            unitId:'militia',
            investedXP:0,
            locked:false
        }; c.state.armySlots = ['militia', null, null, null, null];
        const started = startLegacy(c, {
            type: 'start',
            stage: 1
        });
        ...
        const base = {
            ...c,
            state: started.state,
            battle_clock: started.battleClock
        };
        ...
        expect(ended).toEqual(executeKingdomCommand({
            ...base,
            server_now
        }, { type: 'tick' }));
    });
    ...
    it('collects only the trusted pending victory, including after offline completion', () => {
        ...
        const offline = {
            ...active,
            server_now: '2026-09-05T13:00:00Z'
        };
        const won = executeKingdomCommand(offline, { type: 'tick' });
        const pending = {
            ...offline,
            state: won.state,
            battle_clock: won.battleClock
        };
        expect(pending.state.gold).toBe(0);
        expect(() => executeKingdomCommand(pending, {
            type: 'start',
            stage: 2
        })).toThrow(/Collect/);
        expect(() => executeKingdomCommand(pending, {
    ...
    it('accepts only army intent and validates eligibility against trusted ownership', () => {
        const command = parseKingdomCommand({
            type: 'army',
            slots: ['hatchling', null, null, null, null],
            damage: 999,
            rulesVersion: 1
        });
        expect(command).toEqual({
    ...
    it('catches up identically across fractional polling and absence using frozen stats', () => {
        ...
        const base = {
            ...c,
            state: start.state,
            battle_clock: start.battleClock
        };
        let split = base;
        ...
        const absent = executeKingdomCommand({
            ...base,
        ...
        expect(() => executeKingdomCommand(base, {
            type: 'army',
    ...
    it('repeated requests without elapsed server time cannot speed up combat', () => {
        ...
        for(let i=0;i<100;i++) {
            ...
            next={
                ...next,
                state:result.state,
                battle_clock:result.battleClock
            };
        }
        ...
        const later=executeKingdomCommand({
            ...next,
            server_now:'2026-09-05T12:00:05Z'
        },{type:'tick'});
    ...
    it('recruits and resolves an offline battle using stored building stats', () => {
        ...
        c.state=applyAction(c.state,{
            type:'start',
            stage:1
        }); c.battle_clock=c.server_now;
        ...
        const retried = executeKingdomCommand({
            ...c,
            state: result.state,
            battle_clock: result.battleClock
        }, { type: 'tick' });
