...
function ready() {
    let s=newKingdom();s.tokens.Life=13;s.tokens['Earth & Space']=13;s=applyAction(s,{
        type:'building',
        id:'barracks'
    });
    s=applyAction(s,{
    ...
    return applyAction(s,{
        type:'army',
        slots:['first-0',null,null,null,null]
    });
}
...
describe('Permanent recruitment and battle lifecycle',()=>{
    it('spawns automatically, preserves all owned recruits through victory, and collects Gold once',()=>{
        ...
        expect(()=>applyAction(end,{
            type:'start',
            stage:2
        })).toThrow(/Collect/);
        const paid=applyAction(end,{
            type:'collect-battle',
            stage:1
        });expect(paid.gold).toBe(60);expect(applyAction(paid,{
            type:'collect-battle',
            stage:1
        })).toBe(paid);
        expect(()=>applyAction(paid,{
            type:'start',
            stage:3
        })).toThrow(/next unbeaten/);expect(applyAction(paid,{
            type:'start',
            stage:2
        }).battle!.elapsed).toBe(0);
    });
    it('preserves independent spawn cadence and field limits across all five flexible slots',()=>{
        const s=newKingdom();s.castle=3;
        BUILDINGS.forEach((b,i)=>{
            s.buildings[b.id]=1;s.units[b.unitId]={
                unitId:b.unitId,
                investedXP:0,
                locked:false
            };s.armySlots[i]=b.unitId;
        });
        let started=applyAction(s,{
            type:'start',
            stage:1
        });const b=started.battle!;b.nextEnemy=450;
        b.fighters=Array.from({length:ARMY_LIMIT},(_,i)=>({
            ...unitStats('militia',1),
            kind:'militia',
            side:'player',
            x:5,
    ...
    it('freezes combat against roster edits, keeps battle speed, and resumes identical simulation after reload',()=>{
        let s=applyAction(ready(),{
            type:'start',
            stage:1
        });expect(s.battle!.nextSpawn).toEqual({0:18});
        for(let i=0;i<80;i++){
        ...
        expect(()=>applyAction(s,{type:'castle'})).toThrow(/battle/);expect(()=>applyAction(s,{
            type:'army',
    ...
    it('handles defeat, retreat, simultaneous destruction and timeout without consuming owned units',()=>{
        ...
        s=applyAction(s,{
            type:'start',
            stage:1
        });s.battle!.elapsed=449.75;s.battle!.nextEnemy=450;s.battle!.nextSpawn.militia=450;
        s=applyAction(s,{type:'tick'});expect(s.battle!.result).toBe('draw');expect(s.units).toEqual(owned);
        s=applyAction(s,{
            type:'start',
            stage:1
        });s.battle!.playerHp=1;s.battle!.enemyHp=1;s.battle!.nextId=3;
    s.battle!.fighters=[{
        ...unitStats('militia',1),
        id:1,
        kind:'militia',
...
describe('Castle persistence',()=>{
    ...
    it('persists pending Gold and failed writes, then recovers the same committed collection',async()=>{
        ...
        await expect(changeKingdom('alice',{
            type:'collect-battle',
            stage:1
        })).rejects.toThrow(/not been applied/);fail.mockRestore();expect(loadKingdom('alice')).toEqual(won);
        await changeKingdom('alice',{
            type:'collect-battle',
            stage:1
        },'collect');await changeKingdom('alice',{
            type:'collect-battle',
            stage:1
        },'collect');expect(loadKingdom('alice').gold).toBe(60);expect(loadKingdom('bob').gold).toBe(0);
    });
