...
const answer = (s: Kingdom, id: string, correct: boolean, now = '2026-09-08T12:00:00Z') => applyAction(s,{
    type:'answer',
    id,
    topic:'Life',
    correct
...
describe('Independent deployment groups', () => {
    const army = (swarm=false) => {
        ...
        for(let i=0;i<5;i++){
            s.units[`copy-${i}`]={
                unitId:swarm?'hatchling':'militia',
                investedXP:i*20,
                locked:false
    ...
    it('allows repeated types, rejects the same copy twice, and preserves five independent timers on reload', () => {
        ...
        expect(()=>applyAction(s,{
            type:'army',
            slots:['copy-0','copy-0',null,null,null]
        })).toThrow(/separate/);
