...
describe('Five class progression and combat', () => {
    ...
    it.each(UNITS.map(u=>[u.id] as const))('%s acts and survives snapshot reload with level-10 training', id => {
        const source=fighter(id,1), ally={
            ...fighter('champion',2),
            hp:10,
            x:46,
            cooldown:3
        ...
        full.units[id]={
            unitId:id,
            investedXP:540*3**(unitDefinition(id).tier-1),
            locked:false
        };full.armySlots=[id,null,null,null, null];
    ...
    it('makes a fresh next tier beat a level-5 predecessor', () => {
        for(const unit of UNITS.filter(u=>u.tier>1&&u.unitClass!=='healer')){
            ...
            const old=unitStats(prior.id,1,10,undefined,{
                ...initialUnitProgress(),
                level:5
            });
    ...
    it('scales healing and lifetime budgets by 3× and excludes every healer tier', () => {
        ...
        for(let i=0;i<healers.length;i++){
            ...
            const a={
                    ...fighter(healers[i].id,1),
                    healingLeft:.5
                }, ally={
                    ...fighter('militia',2),
                    hp:10
                };
            const s=step(arena([a,ally,...healers.map((h,j)=>({
                ...fighter(h.id,j+3),
                hp:1,
                healingLeft:0
            }))]));
            ...
            const dead=step(arena([fighter(healers[i].id,1),{
                ...ally,
                hp:.1,
                x:46
            },fighter('champion',3,'enemy',47)]));
            expect(hp(dead,2)).toBe(0);
        }
    });
    it('splashes every enemy in radius and applies the unit penalty to secondary hits', () => {
        const a=fighter('ballista',1);
        expect(Object.keys(unitDefinition('ballista').ability).sort()).toEqual(['description','family','interval']);
        expect(a.splash).toEqual({
            radius: 4,
            fraction: .35
        });
        const targets=[47,47.5,48,48.5,49,49.5,50,52].map((x,i)=>({
        ...
        expect(1000-hp(s,8)).toBeCloseTo(a.damage*3*.75*.35);
        expect(hp(s,9)).toBe(1000);
    });
    it('agrees across catch-up and reload and bounds a full 48-fighter field', () => {
        let s=arena(Array.from({length:48},(_,i)=>({
            ...fighter(UNITS[i%25].id,i+1,i<24?'player':'enemy',i<24?45:48),
            hp:100000,
            maxHp:100000
        })));
        const base={
