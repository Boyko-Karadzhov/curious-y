...
describe('Authoritative ability families', () => {
    it.each(UNITS.map(u => [u.id] as const))('%s acts on the field and round-trips its snapshot', id => {
        const attacker = fighter(id,1); const ally = {
            ...fighter('swordsman',2),
            hp:10,
            x:46
        };
        const enemy = {
            ...fighter('knight',3,'enemy',48),
            hp:1000,
            maxHp:1000
        };
    ...
    it('persists four shots through reload; the fifth bypasses armor and pierces only two targets behind', () => {
        const gun = {
            ...fighter('clockwork-gunner',1),
            attackCount:3
        };
        const enemies = [49,52,55,58].map((x,i) => ({
            ...fighter('shieldbearer',i+2,'enemy',x),
            hp:1000,
            maxHp:1000,
            speed:.01,
            damage:.01
    ...
    it('charge is first attack only; execute requires below half health', () => {
        const lancer=fighter('lancer',1), enemy={
            ...fighter('archer',2,'enemy',48),
            hp:1000,
            maxHp:1000,
            damage:.01
        };
        ...
        expect(400-hp(step(arena([berserker,{
            ...enemy,
            hp:400
        }])),2)).toBeCloseTo(berserker.damage*1.8);
    ...
    it('healing is capped, excludes healers and Keeps, and cannot resurrect', () => {
        const medic=fighter('medic',1), ally={
                ...fighter('swordsman',2),
                hp:64
            }, other={
                ...fighter('medic',3),
        ...
        s=step(arena([medic,{
            ...
            hp:.1,
            x:46
        },fighter('knight',4,'enemy',47)])); expect(hp(s,2)).toBe(0);
    ...
    it('slow and rally are capped, expire and apply after simultaneous decisions', () => {
        ...
        const sage=fighter('battle-sage',1); const sword=fighter('swordsman',2); const enemy={
            ...fighter('archer',3,'enemy',47),
            hp:1000,
            maxHp:1000
        };
    ...
    it('resolves simultaneous lethal hits and castle destruction; ignores dead/out-of-reach preferred targets', () => {
        ...
        const spear=fighter('spearman',1);expect(rosterTarget(spear,[spear,{
            ...b,
            hp:0
        },fighter('knight',3,'enemy',90),fighter('archer',4,'enemy',48)])!.id).toBe(4);
    });
    it('agrees across polling, catch-up and reload for the expanded abilities and keeps field caps', () => {
        ...
        const base={
            ...
            server_now:'2026-09-06T00:01:30Z'
        };
    ...
    it('sustains the full 48-fighter field for 360 bounded simulation steps', () => {
        let s=arena(Array.from({length:48},(_,i)=>({
            ...fighter(UNITS[i%20].id,i+1,i<24?'player':'enemy',i<24?45:48),
            hp:100000,
            maxHp:100000
        })));
