...
vi.mock('../lib/kingdom/equipmentArt',()=>({
    loadEquipmentArtwork:vi.fn(async()=>{}),
    drawEquippedUnit:vi.fn(()=>true)
}));
beforeEach(()=>{
    vi.clearAllMocks();
...
describe('Equipped unit portraits',()=>{
    it('renders the actual identity with equipment, updates tiers and restores the ordinary portrait when gear is sold',async()=>{
        const view=render(<UnitPortrait id="hatchling" equipment={{
            weapon:2,
            armor:3
        }}/>);
        const img=()=>view.container.querySelector('img')!;
        await waitFor(()=>expect(img().src).toContain('data:image/png'));
        expect(drawEquippedUnit).toHaveBeenLastCalledWith(expect.anything(),'hatchling',{
            weapon:2,
            armor:3
        },0,120);
        view.rerender(<UnitPortrait id="hatchling" equipment={{
            weapon:4,
            armor:5
        }}/>);
        expect(img().getAttribute('src')).toBe('/assets/units/hatchling-v2/portrait.png');
        await waitFor(()=>expect(drawEquippedUnit).toHaveBeenLastCalledWith(expect.anything(),'hatchling',{
            weapon:4,
            armor:5
        },0,120));
        view.rerender(<UnitPortrait id="hatchling" equipment={{
    ...
    it('ignores an old asynchronous portrait after switching unit identity',async()=>{
        ...
        const view=render(<UnitPortrait id="stinger" size={81} equipment={{
            weapon:1,
            armor:1
        }}/>);
    ...
    it('keeps Siege bodies and displays ammunition only',()=>{
        const view=render(<UnitPortrait id="catapult" equipment={{
            weapon:5,
            armor:5
        }}/>);
    ...
    it('resolves gear by class and excludes artifact and Siege armor visuals',()=>{
        ...
        state.forge.equipped['swarm:armor']={
            id:'armor',
            unitClass:'swarm',
            slot:'armor',
            tier:4,
        ...
        state.forge.equipped['swarm:artifact']={
            id:'artifact',
            unitClass:'swarm',
            slot:'artifact',
            tier:5,
        ...
        state.forge.equipped['siege:armor']={
            id:'doctrine',
            unitClass:'siege',
            slot:'armor',
            tier:5,
        ...
        for(const id of ['hatchling','stinger','ravager'] as const){
            expect(portraitEquipment(state,id)).toEqual({
                weapon:0,
                armor:4
            });
        }

        expect(portraitEquipment(state,'catapult')).toEqual({
            weapon:0,
            armor:0
        });
    });
    it('uses frozen gear in an active army and current gear in the collection and next preparation',async()=>{
        ...
        await waitFor(()=>expect(drawEquippedUnit).toHaveBeenCalledWith(expect.anything(),'hatchling',{
            weapon:0,
            armor:2
        },0,120));
