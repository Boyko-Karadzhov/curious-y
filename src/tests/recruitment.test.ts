...
const pack=(s:Kingdom,n:number,draws=[.5,.5,.5,0,0,0])=>applyAction(s,{
    type:'recruit',
    id:'barracks'
},{
    requestId:`pack-${n}`,
...
describe('Unified recruitment and independent copies',()=>{
    it('builds one Hall, charges Earth and Life, keeps three copies and guarantees an attacking start',()=>{
        const s=rich();const built=applyAction(s,{
            type:'building',
            id:'barracks'
    ...
    it('merges only selected same-class spare copies, conserves XP and protects assigned and locked donors',()=>{
        ...
        s.armySlots[0]='pack-1-0';s=applyAction(s,{
            type:'merge',
            recipient:'pack-1-0',
            donors:['pack-2-0','pack-2-1']
        });
        ...
        s.units['pack-2-2'].locked=true;expect(()=>applyAction(s,{
            type:'merge',
            recipient:'pack-1-0',
            donors:['pack-2-2']
        })).toThrow(/locked/);
    ...
    it('resets old military state while preserving wallets and learning evidence',()=>{
        const s=rich();s.gold=42;s.units.a={
            unitId:'militia',
            investedXP:20,
            locked:false
        };
        const next=parseKingdom(JSON.stringify({
            ...s,
            version:10
        }));expect(next.version).toBe(11);expect(next.units).toEqual({});expect(next.gold).toBe(42);expect(next.lifetimeGold).toBe(42);expect(next.tokens).toEqual(s.tokens);
    });
    it('keeps exact uncapped XP boundaries and rejects merge overflow',()=>{
        for(const u of UNITS){
            for(const level of [1,2,3,10,1000,100000]){
                const xp=xpThreshold(level,u.id);expect(recruitLevel({
                    unitId:u.id,
                    investedXP:xp,
                    locked:false
                })).toBe(level);if(level>1){
                    expect(recruitLevel({
                        unitId:u.id,
                        investedXP:xp-1,
                        locked:false
        ...
        const s=rich();s.units={
            a:{
                unitId:'militia',
                investedXP:Number.MAX_SAFE_INTEGER-10,
                locked:false
            },
            b:{
                unitId:'militia',
        ...
        expect(()=>applyAction(s,{
            ...
            recipient:'a',
            donors:['b']
        })).toThrow(/safe integer/);expect(xpProgress(s.units.a).current).toBeLessThan(xpProgress(s.units.a).required);
    });
...
describe('Recruitment persistence',()=>{
    ...
    it('recovers the same copies and reveal across reload, account isolation and reset',async()=>{
        ...
        const epoch=demoGeneration('demo'),action={
            type:'recruit' as const,
            id:'barracks' as const
        };const first=await changeKingdom('demo',action,'same',epoch);
    ...
    it('uses reserved randomness and server time in the authoritative executor',()=>{
        const s=rich();s.buildings.barracks=1;const context={
                state:s,
                revision:0,
                generation:0,
                battle_clock:null,
                server_now:'2026-09-08T00:00:00Z'
            },entropy={
                requestId:'reserved',
                draws:[.1,.2,.3,.4,.5,.6]
            };
        expect(executeKingdomCommand(context,{
            type:'recruit',
            id:'barracks'
        },entropy)).toEqual(executeKingdomCommand(context,{
            type:'recruit',
            id:'barracks'
        },entropy));
    });
