...
The playable loop is **learning → Resources → buildings and unit upgrades → stronger armies → battle victories**. Guilds, gacha, equipment, and PvP are outside this phase. Forge is shown only as a future unlock, without crafting controls. See [Castle progression, specialties, Library eligibility and Treasury payouts](docs/castle-progression.md).

- Castle upgrades cost Runes and Influence. Barracks use Force; Archery Range uses Astral Dust and Insight; Stable uses Essence and Reagents; Siege Workshop uses Logic Cores and Force; Academy uses Essence and Insight; Treasury uses Runes and Influence. Combat construction and upgrades cost only Resources. Only Treasury upgrades additionally cost Gold.
- Answers earn versioned **learning-value rewards**: base 20 with correctness, reasoning, first-success, mastered-practice, due-review and successful-boss factors. Repeated low-value attempts taper to zero; learning remains available. Rewards split by validated concept topic weights, and Collect credits the exact saved resource lines. See [worked payouts, review rules and release requirements](docs/learning-value-rewards.md). Resources cannot be exchanged for Gold. Victories award **60 Gold plus 10 per stage after the first**, plus a snapshotted Treasury bonus of 2% per level (10% maximum, bonus rounded down); defeats, draws, and retreats award none. Reopening history does not award Resources. Each question attempt has a stable reward ID to prevent duplicate collection.
- Start at Keep (stored Castle) level 1 with an empty Gold balance. A first correct direct-inference answer with 100% Physics weight earns 25 Force, funding the 10-Force Barracks with 15 left over; mixed-topic payouts vary. Castle upgrades cost `10 × current level` each of Runes and Influence and add 120 castle HP, up to level 5.
- Buildings unlock their unit permanently. Assign units manually through the four army squares; clicking a square opens its stats and unit choices. Available units prompt you toward the next empty square. Resource cost is `base resource cost × next building level`, with `20 × current building level` Gold added only for Treasury; building levels cannot exceed the Castle. Each level after the first retains 30% base HP/damage growth and adds its branch specialty. Academy unlocks a limited-budget Medic. Library levels at 10/30/75/150 verified qualifying concepts add up to 4% army HP; they cannot be purchased.

| Building | Unit | Castle required | Base resource cost | Spawn interval |
| --- | --- | --- | --- | --- |
| Barracks | Swordsman | 1 | 10 Force | 4.5 seconds |
| Archery Range | Archer | 1 | 15 Astral Dust + 15 Insight | 6 seconds |
| Stable | Knight | 2 | 20 Essence + 20 Reagents | 9 seconds |
| Siege Workshop | Catapult | 3 | 30 Logic Cores + 30 Force | 12 seconds |
| Academy | Medic | 2 | 20 Essence + 20 Insight | 12 seconds |

Battle is the first section in Castle, with castle health, stage/time, retreat, unit counts, and animated spawn rings overlaid on the battlefield. Start and result controls appear in a large centered battlefield overlay. Press Start battle to fight the next unbeaten stage: 1-1 through 1-10, then 2-1 and onward. Victory offers Collect for its Gold, then Next battle. Pending Gold persists across reloads and must be collected before another battle; defeat, retreat, or a draw offers Retry for the same stage. Every attempt requires a button press, and cleared stages cannot be replayed. Enemy castle health and army strength increase with stage number. Each equipped unit waits its full listed spawn interval before its first recruit, then continues independently on that interval. Victory Collect flies Gold coins into the HUD balance after the reward is saved. At most 24 allied units can be on the field; the oldest due spawn takes the next free space without accumulating missed spawns. Both armies advance and fight automatically. Archers fight at range, Knights are fast and durable, and Catapults deal triple damage to castles. New battles run at 5× speed with an 90-second wall-time limit (450 simulation seconds); saved legacy battles retain their original rules. See [campaign balance and measured progression](docs/battle-balance.md). Both castles surviving at the time limit, or both falling together, produces a draw. Battles cost no Gold, and defeat and retreat preserve buildings, Resources, and Gold.

**Persistence:** Signed-in Castle wallets, buildings, and campaigns are stored in Supabase. Edge commands validate every purchase and battle action; transactions serialize rewards and resource changes. Recruitment and battle time are measured by the server and continue while away. Explorer Demo saves locally, runs combat while the app is visible (including while learning), and resumes automatically when reopened. Existing battle saves preserve their fighters and cleared-stage count; the old supply pool is removed and each spawn timer starts one interval after the saved battle time. Retrying an answer recovers its committed result without crediting twice. Reset Progress atomically clears account progress and invalidates in-flight question generation.

**Try it:** Start Explorer Demo, choose the initial Barracks goal’s Physics action, answer correctly, click **Collect**, then spend 10 Force to build Barracks, then start Battle 1-1 and watch Swordsmen spawn and fight on their own. You can return to learning while the battle runs. No API key is needed for this loop. Demo questions and scripted tutor replies are visibly labeled. Live generation requires Google sign-in and a Gemini key saved in Settings; these controls are disabled and marked unavailable in Explorer Demo.

**Merged UI:** The incoming resource names (Force, Runes, Reagents, Essence, Logic Cores, Astral Dust, Insight, Influence) are the eight Resources, displayed beside Gold at the top. Each topic card shows its primary resource. Resources are spent directly on specific upgrades; the existing `tokens` save keys remain compatible. The redesigned resource bar, reward card, and sidebar use the Phase I state. Tiny Swords artwork follows real combat positions and health instead of a simulated preview. Ranked arena, trophies, seasons, streak bonuses, daily countdowns, gems, and Archive Keys are hidden. The old server `game_stats` schema remains for backend compatibility; it is not the Phase I Castle wallet.

**Collecting Resources:** Answering prepares a reward inside the question card. Click **Collect** to credit the Castle wallet; only then is **Next Question** available. Pending rewards survive refreshes (in Supabase for signed-in accounts and local storage for Explorer Demo). Collection is idempotent, and existing rewards remain collected. Apply `20260905233000_collect_learning_rewards.sql` and deploy the updated `learning` Edge Function before releasing this frontend. Collection flies resource seals into the matching HUD balance, with a reduced-motion alternative.
...
`src/tests/kingdom.test.ts` covers economy rules, automatic recruitment, independent spawn timers and field limits, all units and chapter progression, stronger armies changing combat outcomes, results, retries, reward deduplication, save failures, and account isolation. `src/tests/KingdomJourney.test.tsx` exercises the answer-to-battle UI, combat during learning, automatic resumption after reload, history, reset, and stale question requests. Deploy the updated `learning` Edge Function and frontend, and apply `20260905220000_resource_economy.sql`, together to enable the resource economy for signed-in accounts.
