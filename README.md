...
The playable loop is **topic tokens → Gold → Castle → four buildings → four units → Battle**. Guilds, gacha, equipment, and PvP are outside this phase and have no UI entry points.
...
**Try it:** Start Explorer Demo, answer a question, visit Castle, exchange tokens, build Barracks, then start Battle 1-1 and watch Swordsmen spawn and fight on their own. You can return to learning while the battle runs. No API key is needed for this loop. Demo questions and scripted tutor replies are visibly labeled. Live generation requires Google sign-in and a Gemini key saved in Settings; these controls are disabled and marked unavailable in Explorer Demo.
...
`src/tests/kingdom.test.ts` covers economy rules, automatic recruitment, independent spawn timers and field limits, all units and chapter progression, stronger armies changing combat outcomes, results, retries, reward deduplication, save failures, and account isolation. `src/tests/KingdomJourney.test.tsx` exercises the answer-to-battle UI, combat during learning, automatic resumption after reload, history, reset, and stale question requests. Deploy the updated `learning` Edge Function along with the frontend to enable sequential stages and independent spawn timers for signed-in accounts.
