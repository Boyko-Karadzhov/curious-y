...
Five progressively stronger PvE fronts unlock in order. Start a battle and built buildings automatically recruit units in rotation: Swordsman, Archer, Knight, then Catapult, skipping unbuilt buildings. The opening army spawns immediately from the initial 10 supply. Recruitment waits for the next unit's cost, so expensive units get their turn. Both armies advance, attack enemy units, and damage the opposing castle automatically. Archers fight at range, Knights are fast and durable, and Catapults deal triple damage to castles. Supply regenerates at 2/second and caps at 20; at most 24 allied units can be on the field, with recruitment resuming as spaces open. Destroy the enemy castle within 120 seconds to win. Both castles surviving at the time limit, or both falling together, produces a draw. Defeat and retreat preserve buildings and currencies. Battles are free to retry and replay; they award campaign progress, not Gold.

**Persistence:** Signed-in Castle wallets, buildings, and campaigns are stored in Supabase. Edge commands validate every purchase and battle action; transactions serialize rewards and resource changes. Recruitment and battle time are measured by the server and continue while away. Explorer Demo saves locally, runs combat while the app is visible (including while learning), and resumes automatically when reopened. Existing battle saves preserve their fighters and supply when adopting automatic recruitment. Retrying an answer recovers its committed result without crediting twice. Reset Progress atomically clears account progress and invalidates in-flight question generation.

**Try it:** Start Explorer Demo, answer a question, visit Castle, exchange tokens, build Barracks, then start Meadow Outpost and watch Swordsmen spawn and fight on their own. You can return to learning while the battle runs. No API key is needed for this loop. Demo questions and scripted tutor replies are visibly labeled. Live generation requires Google sign-in and a Gemini key saved in Settings; these controls are disabled and marked unavailable in Explorer Demo.
...
`src/tests/kingdom.test.ts` covers economy rules, automatic recruitment, supply and field limits, all units and fronts, stronger armies changing combat outcomes, results, retries, reward deduplication, save failures, and account isolation. `src/tests/KingdomJourney.test.tsx` exercises the answer-to-battle UI, combat during learning, automatic resumption after reload, history, reset, and stale question requests. Deploy the updated `learning` Edge Function along with the frontend to enable automatic recruitment for signed-in accounts.
