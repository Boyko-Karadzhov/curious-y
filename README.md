...
- A new answer earns **10 Resources when correct** or **3 for an incorrect learning attempt**, in that question's topic. Resources cannot be exchanged for Gold. Victories award **60 Gold plus 10 per stage after the first**; defeats, draws, and retreats award none. Reopening history does not award Resources. Each question attempt has a stable reward ID to prevent duplicate collection.
- Start at Castle level 1 with an empty treasury. One correct Physics answer funds the first Barracks. Castle upgrades cost `60 × current level` Gold plus `10 × current level` each of Runes and Influence and add 120 castle HP, up to level 5.
