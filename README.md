...
The playable loop is **learning → Resources → buildings → units → battle victories → Gold → upgrades**. Guilds, gacha, equipment, and PvP are outside this phase. Forge is shown only as a future unlock, without crafting controls. See [Castle progression, specialties, Library eligibility and Treasury payouts](docs/castle-progression.md).

- Castle upgrades cost Gold, Runes, and Influence. Barracks use Force; Archery Range uses Astral Dust and Insight; Stable uses Essence and Reagents; Siege Workshop uses Logic Cores and Force; Academy uses Essence and Insight; Treasury uses Runes and Influence. Initial construction costs only Resources; subsequent building upgrades also cost Gold.
