-- Territory economy, independent owned copies, unified recruitment and swarm.
-- Development reset of military/campaign/equipment; learning and wallets remain.
ALTER TABLE public.kingdom_state ALTER COLUMN state SET DEFAULT '{"version":11,"forge":{"count":0,"pending":null,"equipped":{}},"discovered":[],"units":{},"towers":{"rule":"earned-proficiency-v1","points":{"force":0,"runes":0,"reagents":0,"essence":0,"cores":0,"astral":0,"insight":0,"influence":0}},"libraryConcepts":0,"armySlots":[null,null,null,null,null],"gold":0,"tokens":{"Physics":0,"Mathematics & Logic":0,"Chemistry":0,"Life":0,"Computer Science":0,"Earth & Space":0,"Mind & Behavior":0,"Society & History":0},"castle":1,"buildings":{"barracks":0,"range":0,"stable":0,"workshop":0,"academy":0,"treasury":0,"library":0,"forge":0},"rewarded":[],"cleared":0,"battle":null,"recruitCount":{"barracks":0},"lastResult":null,"lifetimeGold":0,"tribute":{"day":"","territories":0,"correct":false,"claimed":false,"paid":0},"doctrine":"balanced"}'::jsonb;
...
UPDATE public.kingdom_state SET state='{"version":11,"forge":{"count":0,"pending":null,"equipped":{}},"discovered":[],"units":{},"towers":{"rule":"earned-proficiency-v1","points":{"force":0,"runes":0,"reagents":0,"essence":0,"cores":0,"astral":0,"insight":0,"influence":0}},"libraryConcepts":0,"armySlots":[null,null,null,null,null],"gold":0,"tokens":{"Physics":0,"Mathematics & Logic":0,"Chemistry":0,"Life":0,"Computer Science":0,"Earth & Space":0,"Mind & Behavior":0,"Society & History":0},"castle":1,"buildings":{"barracks":0,"range":0,"stable":0,"workshop":0,"academy":0,"treasury":0,"library":0,"forge":0},"rewarded":[],"cleared":0,"battle":null,"recruitCount":{"barracks":0},"lastResult":null,"lifetimeGold":0,"tribute":{"day":"","territories":0,"correct":false,"claimed":false,"paid":0},"doctrine":"balanced"}'::jsonb || jsonb_build_object('tokens',state->'tokens','gold',state->'gold','lifetimeGold',state->'gold','towers',state->'towers','libraryConcepts',state->'libraryConcepts','buildings',('{"barracks":0,"range":0,"stable":0,"workshop":0,"academy":0,"treasury":0,"library":0,"forge":0}'::jsonb || jsonb_build_object('library',state->'buildings'->'library'))), generation=generation+1,revision=revision+1,battle_clock=NULL,issuance_lease=NULL,issuance_until=NULL,progression_goal=DEFAULT,goal_revision=goal_revision+1;

CREATE OR REPLACE FUNCTION public.roster_unit_id(p_id text) RETURNS boolean LANGUAGE sql IMMUTABLE SET search_path='' AS $$
...
$$;
CREATE OR REPLACE FUNCTION public.recruitment_class(id text) RETURNS text LANGUAGE sql IMMUTABLE SET search_path='' AS $$
...
END $$;
CREATE OR REPLACE FUNCTION public.valid_forged_item(i jsonb) RETURNS boolean LANGUAGE plpgsql IMMUTABLE SET search_path='' AS $$
...
EXCEPTION WHEN OTHERS THEN RETURN false;
END $$;
CREATE OR REPLACE FUNCTION public.valid_forge_state(s jsonb) RETURNS boolean LANGUAGE plpgsql IMMUTABLE SET search_path='' AS $$
...
-- UTC days are server-owned. No missed-day accumulation and no Treasury gate.
CREATE FUNCTION public.apply_territory_tribute(s jsonb, p_now timestamptz, p_cleared bigint DEFAULT NULL, p_correct boolean DEFAULT false)
RETURNS jsonb LANGUAGE plpgsql IMMUTABLE SET search_path='' AS $$
DECLARE t jsonb:=s->'tribute'; day text:=to_char(p_now AT TIME ZONE 'UTC','YYYY-MM-DD'); amount bigint;
BEGIN
 IF t->>'day' IS DISTINCT FROM day THEN
   t:=jsonb_build_object('day',day,'territories',(s->>'cleared')::bigint,'correct',false,'claimed',false,'paid',0);
 END IF;
 IF p_correct THEN t:=jsonb_set(t,'{correct}','true'); END IF;
 IF (s->>'cleared')::bigint=0 AND COALESCE(p_cleared,0)>0 THEN t:=jsonb_set(t,'{territories}','1'); END IF;
 IF (t->>'correct')::boolean AND NOT (t->>'claimed')::boolean AND (t->>'territories')::bigint>0 THEN
   amount:=floor((t->>'territories')::numeric*10*(100+least(5,(s->'buildings'->>'treasury')::integer)*2)/100);
   s:=jsonb_set(s,'{gold}',to_jsonb((s->>'gold')::bigint+amount));
   s:=jsonb_set(s,'{lifetimeGold}',to_jsonb((s->>'lifetimeGold')::bigint+amount));
   t:=t||jsonb_build_object('claimed',true,'paid',amount);
 END IF;
 RETURN jsonb_set(s,'{tribute}',t);
END $$;
REVOKE ALL ON FUNCTION public.apply_territory_tribute(jsonb,timestamptz,bigint,boolean) FROM PUBLIC,anon,authenticated;
GRANT EXECUTE ON FUNCTION public.apply_territory_tribute(jsonb,timestamptz,bigint,boolean) TO service_role;

CREATE FUNCTION public.credit_correct_answer_tribute() RETURNS trigger LANGUAGE plpgsql SECURITY DEFINER SET search_path='' AS $$
DECLARE k public.kingdom_state%ROWTYPE; next_state jsonb; answered timestamptz;
BEGIN
 IF NEW.reward->>'correct' IS DISTINCT FROM 'true' THEN RETURN NEW; END IF;
 SELECT * INTO STRICT k FROM public.kingdom_state WHERE user_id=NEW.user_id FOR UPDATE;
 IF k.generation<>NEW.generation THEN RAISE EXCEPTION 'Question generation changed.'; END IF;
 SELECT answered_at INTO STRICT answered FROM public.questions WHERE id=NEW.question_id AND user_id=NEW.user_id;
 next_state:=public.apply_territory_tribute(k.state,answered,NULL,true);
 IF next_state IS DISTINCT FROM k.state THEN
   UPDATE public.kingdom_state SET state=next_state,revision=revision+1 WHERE user_id=NEW.user_id;
 END IF;
 RETURN NEW;
END $$;
REVOKE ALL ON FUNCTION public.credit_correct_answer_tribute() FROM PUBLIC,anon,authenticated,service_role;
CREATE TRIGGER correct_answer_tribute AFTER INSERT ON public.learning_reward_events FOR EACH ROW EXECUTE FUNCTION public.credit_correct_answer_tribute();
...
BEGIN
  ...
  IF p_request_id IS NULL OR COALESCE(p_command->>'type','') NOT IN ('castle','building','army','start','tick','retreat','collect-battle','recruit','forge','resolve-forge','merge','lock','doctrine')
    OR (p_command->>'type'='building' AND COALESCE(p_command->>'id','') NOT IN ('barracks','academy','treasury','forge'))
    ...
    OR p_state->'tribute' IS DISTINCT FROM (public.apply_territory_tribute(k.state,now(),(p_state->>'cleared')::bigint,false)->'tribute')
