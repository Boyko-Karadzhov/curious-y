...
Existing graph (data): ${JSON.stringify(existing.filter(n => n.kind === 'concept').map(n => ({ id: n.id, title: n.title, definition: n.definition })))}`, matchSchema, value => validateMatches(value, names, existing));
}
