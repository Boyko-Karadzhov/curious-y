...
export async function expandCurriculum(context: LearningContext, graph: LearningGraph & { generation: number }, topic: string) {
    const reservation = await context.rpc<DraftLease>('begin_curriculum_stage', { p_topic: topic, p_generation: graph.generation });
    try {
...
async function prepareStage(context: LearningContext, reservation: DraftLease, topic: string) {
    ...
    await context.rpc('save_curriculum_stage', {
        p_topic: topic, p_lease: reservation.lease, p_generation: reservation.graph.generation, p_draft: draft,
    });
    return { preparing: true as const, topic, generation: reservation.graph.generation };
}
