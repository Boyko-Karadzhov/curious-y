...
import { FACETS, FACET_ORDER, journeyView, validateJourneyPlan, type JourneyPlan, type SavedJourney, type JourneyNode, type Facet, type JourneyProgress } from '../_shared/journey.ts';
...
const planSchema = {
  type: 'OBJECT', properties: {
    ...
    }, required: ['id', 'title', 'definition', 'kind', 'facets', 'requires'] } },
...
export function journeyQuestionPrompt(plan: JourneyPlan, node: JourneyNode, facet: Facet, progress: JourneyProgress, history: string[]): string {
  const known = plan.nodes.filter(n => n.id !== node.id && (progress[n.id]?.intuition?.successes ?? 0) >= 2 && (progress[n.id]?.mechanism?.successes ?? 0) >= 2);
...
export function validateJourneyQuestion(value: unknown, plan: JourneyPlan, node: JourneyNode, progress: JourneyProgress, history: string[]) {
  ...
  const known = new Set(plan.nodes.filter(n => n.id !== node.id && (progress[n.id]?.intuition?.successes ?? 0) >= 2 && (progress[n.id]?.mechanism?.successes ?? 0) >= 2).map(n => n.title));
...
Start with one ambitious but accessible synthesis boss QUESTION and plan backwards. Use 4-8 concept nodes plus exactly one boss. All nodes must contribute to the boss. Use at least two starting roots, no cycles, unique lowercase hyphenated ids. Every concept must include intuition and mechanism; add applicable dimensions from ${FACET_ORDER.join(', ')}. No artificial math or infinite limits. Every dependency specifies the parent nodeId and which facets must be confirmed. Boss requires at least two concepts. Include at least one concept with multiple parents. Foundations assume only ordinary experience and the earned knowledge above; bridge every new technical term. Do not reuse a previous concept title. Definitions are private author context. The chapter title should invite curiosity WITHOUT revealing the boss. Nodes have kind concept or boss. Give the boss only the mechanism facet. Return the exact topic ${topic} and requested JSON.`;
      ...
      for (let attempt = 0; attempt < 3; attempt++) {
        try {
          ...
          plan = { ...plan, priorKnowledge: priorKnowledge.slice(-60) };
          break;
        }
  ...
  if (typeof body.journeyId !== 'string' || typeof body.nodeId !== 'string' || !FACET_ORDER.includes(body.facet as Facet)) throw new Error('Choose a concept and dimension on your map.');
