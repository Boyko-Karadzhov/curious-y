import { DIMENSION_ORDER, DIMENSIONS, type ConceptNode, type Dimension } from '../_shared/journey.ts';
...
/** One content contract for planning, teaching and questions. */
export const DIMENSION_GUIDANCE: Record<Dimension, string> = {
...
function dimensionRequirements(): string {
    return DIMENSION_ORDER.map(dimension => `${dimension} (${DIMENSIONS[dimension].label}): ${DIMENSION_GUIDANCE[dimension]}`).join('\n');
}
