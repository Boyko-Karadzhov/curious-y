import type { JourneyPlan } from '../_shared/journey.ts';
...
export function journeyAuditPrompt(plan: JourneyPlan): string {
  return `Audit the teachability and factual accuracy of this ONE small chapter, not an entire subject curriculum.
${BASIC_CONCEPT_RULE}
Scope: one coherent subarea is sufficient. A chapter about fungi does not need animals, anatomy, medicine, or genetics. Breadth develops across future chapters; missing unrelated subfields is NEVER a blocker or a reason to add nodes here.
...
Curriculum data: ${JSON.stringify(plan)}`;
}
