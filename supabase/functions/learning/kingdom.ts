...
export function parseKingdomCommand(value: unknown): Exclude<Action, { type: 'answer' }> {
    if (!value || typeof value !== 'object' || Array.isArray(value)) {
        throw new Error('Invalid Castle command.');
    }
    const input = value as Record<string, unknown>;
    switch (input.type) {
        case 'merge':
            if (typeof input.recipient !== 'string' || !Array.isArray(input.donors) || input.donors.length > 1000 || !input.donors.every(id => typeof id === 'string')) {
...
export function executeKingdomCommand(context: CommandContext, command: Exclude<Action, { type: 'answer' }>, entropy?: ActionEntropy) {
    ...
    if (state.battle && !state.battle.result && clock) {
        ...
        for (let i = 0; i < steps && state.battle && !state.battle.result; i++) {
