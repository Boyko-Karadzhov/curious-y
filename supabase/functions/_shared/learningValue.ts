    ...
    return {
        ...
        calculation: {
            ...
            limits: {
                minimum: t.minimum,
                maximum: t.maximum,
                lowValueMaximum: t.lowValueMaximum,
                lowValueFactors: [...t.lowValueFactors]
            },
            firstSuccess,
            due,
            lowValue,
...
export function advanceReview(successes: number, step: number, nextDueAt: string | null, correct: boolean, answeredAt: string, atomic = false) {
    ...
    if (!correct) {
        return {
            reviewStep: 0,
            nextDueAt: successes > 0 ? new Date(Date.parse(answeredAt) + tuning.reviewDays[0] * 86400000).toISOString() : null
        };
    }
    ...
    return {
        reviewStep,
        nextDueAt: new Date(Date.parse(answeredAt) + tuning.reviewDays[reviewStep] * 86400000).toISOString()
    };
}
