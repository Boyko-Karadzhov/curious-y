...
export const BASE_GOLD = balance.economy.dailyProduction.baseGold;
export const FOOD_PER_FARM_LEVEL = balance.economy.dailyProduction.foodPerFarmLevel;
export const METAL_PER_MINUTE_PER_LEVEL = balance.economy.dailyProduction.metalPerMinutePerLevel;
...
export function availableProduction(state: Kingdom, now: string = new Date().toISOString()) {
    ...
    const metal = state.buildings.smelter > 0
        ? Math.min(state.buildings.smelter * balance.economy.dailyProduction.metalStorageMinutes, state.production.metalStored + minutes * state.buildings.smelter * METAL_PER_MINUTE_PER_LEVEL) : 0;
    const gold = state.tribute.day !== utcDay(now) || !state.tribute.claimed
        ? BASE_GOLD + Math.floor(state.cleared * balance.economy.dailyProduction.tributePerTerritory * (100 + Math.min(balance.building.treasury.cap, state.buildings.treasury) * balance.economy.treasury.goldPercentPerLevel) / 100) : 0;
...
function resetMetalClock(state: Kingdom, now: string, metal: number) {
    ...
    state.production.metalAt = metal >= state.buildings.smelter * balance.economy.dailyProduction.metalStorageMinutes
