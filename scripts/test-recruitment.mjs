...
export async function testUnitCollection({db,rpc,check}) {
  ...
  const recruited=await commit(command,id);check(Object.keys(recruited.state.units).length,1);check(recruited.state.tokens.Physics,75);check(recruited.result.recruits.length,3);
  ...
  check(current.state.units[recipient].investedXP,50);check(recruited.result.merge.level,2);check(Object.keys(current.state.units).length,1);
  check((await rpc('kingdom_snapshot',other)).state.units,{});
  for(const type of ['unit-unlock','unit-star','unit-level','merge','lock'])await assert.rejects(rpc('commit_kingdom_command',user,0,current.revision,randomUUID(),{type,id:'militia',expected:1},current.state,null),/Invalid/);
  ...
  await assert.rejects(rpc('commit_kingdom_command',user,0,current.revision,randomUUID(),command,duplicateClass,null),/Invalid/);
...
export async function testUnitRaces({db,pool,rpc,check}) {
  ...
  check(retried.state.tokens.Physics,0);check(Object.keys(retried.state.units).length,1);check(retried.state.recruitCount.barracks,2);
  ...
  check(Object.values(retried.state.units)[0].investedXP,50);
